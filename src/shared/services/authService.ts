...
class DesktopAuthService {
  ...
  private async initializeAuth() {
    try {
      authLogger.debug('🔧 데스크톱 인증 서비스 초기화 시작...');

      // Firebase는 firebaseConfig.ts에서 이미 초기화됨
      authLogger.debug('🔥 Firebase 앱은 firebaseConfig.ts에서 초기화됨');
      ...
      this.onAuthStateChanged(user => {
        ...
        if (user) {
          ...
          // User is signed out.
          authLogger.debug('👣 사용자가 로그아웃되었습니다.');
        }
      });

      this.isInitialized = true;
      authLogger.debug('✅ 데스크톱 인증 서비스 초기화 완료');
    } catch (error) {
      authLogger.error('❌ 인증 서비스 초기화 실패:', error);
  ...
  private async checkForRedirectResult() {
    try {
      authLogger.debug('🔍 Google OAuth Redirect 결과 확인 중...');

      const result = await getRedirectResult(auth);

      if (result && result.user) {
        authLogger.debug('✅ Google OAuth Redirect 성공:', result.user.uid);
        authLogger.debug('📧 이메일:', result.user.email);
        authLogger.debug('👤 이름:', result.user.displayName);
        ...
        authLogger.debug('ℹ️ Google OAuth Redirect 결과 없음 (정상)');
        return null;
      }
    } catch (error) {
      authLogger.error('❌ Google OAuth Redirect 결과 확인 실패:', error);
      return null;
    }
  }
  ...
  private async handleUserAuthenticated(user: User) {
    try {
      ...
      try {
        ...
        authLogger.debug('✅ 백엔드와 사용자 정보 동기화 예정');
      } catch (apiError) {
        authLogger.warn('⚠️ 백엔드 동기화 실패 (계속 진행):', apiError);
      }
      ...
      authLogger.debug('✅ 사용자 인증 처리 완료');
    } catch (error) {
      authLogger.error('❌ 사용자 인증 처리 중 오류:', error);
    }
  }
  ...
  async signInWithGoogle(): Promise<User | null> {
    try {
      authLogger.debug('🔍 Desktop Google 로그인 시도 중...');
      ...
      // 1. 먼저 redirect 결과 확인
      try {
        authLogger.debug('🔍 이전 redirect 결과 확인...');
        const result = await getRedirectResult(auth);
        if (result && result.user) {
          authLogger.debug('✅ Redirect 결과에서 사용자 발견:', result.user.uid);
          return result.user;
        }
      } catch (redirectError) {
        authLogger.debug('ℹ️ Redirect 결과 없음 (정상)');
      }

      // 2. Electron에서는 Popup 대신 바로 Redirect 사용 (중복 창 방지)
      authLogger.debug('🔄 Electron 환경: Redirect 방식 사용 (Popup 방식 건너뜀)...');
      ...
      authLogger.error('❌ Desktop Google 로그인 실패:', error);
  ...
  private async handleOAuthCallback(callbackUrl: string): Promise<User | null> {
    try {
      authLogger.debug('🔄 OAuth Callback 처리 중...');
      ...
      authLogger.debug('✅ Authorization code 받음, Firebase 인증 시도...');

      // 여기서는 간단히 테스트 계정으로 로그인
      return await this.signInWithGoogleFallback();

    } catch (error) {
      authLogger.error('❌ OAuth Callback 처리 실패:', error);
      throw error;
    }
  }
  ...
  private async signInWithGoogleFallback(): Promise<User | null> {
    authLogger.debug('⚠️ Google OAuth 실패 - 앱에서 직접 테스트 로그인을 처리하세요');
    return null;
  }

  private handleGoogleSignInError(error: any) {
    authLogger.error('Google SignIn Error Details:', {
      code: error.code,
    ...
    switch (error.code) {
      ...
      case 'auth/popup-closed-by-user':
        authLogger.debug('사용자가 Google 로그인을 취소했습니다.');
        break;
      case 'auth/popup-blocked':
        authLogger.error('팝업이 차단되었습니다. 브라우저 설정을 확인해주세요.');
        break;
      case 'auth/network-request-failed':
        authLogger.error('네트워크 연결을 확인해주세요.');
        break;
      default:
        authLogger.error('알 수 없는 Google 로그인 오류가 발생했습니다.');
    }
  }
  ...
  async signInWithApple(): Promise<User | null> {
    try {
      authLogger.debug('🍎 Apple 로그인 시도 중...');
      authLogger.debug('⚠️ Apple 로그인은 개발 중입니다. Google 로그인을 사용해주세요.');
      return null;
    } catch (error: any) {
  ...
  async signUp(
    ...
    try {
      authLogger.debug('🚶‍♂️ 회원가입 시도:', email);
      ...
      try {
        ...
        localStorage.setItem('memobee_firebase_user', JSON.stringify(userData));
        localStorage.setItem('memobee_last_login', new Date().toISOString());
        authLogger.debug('✅ 회원가입 데이터 저장 완료');

        // The onAuthStateChanged listener will handle post-authentication
        authLogger.debug('✅ 회원가입 성공');
        return userCredential.user;
      } catch (storageError) {
        authLogger.warn('⚠️ 회원가입 데이터 저장 실패 (회원가입은 성공):', storageError);
        return userCredential.user;
      }
    } catch (error: any) {
  ...
  async signIn(
    ...
    try {
      ...
      try {
        ...
        localStorage.setItem('memobee_firebase_user', JSON.stringify(userData));
        localStorage.setItem('memobee_last_login', new Date().toISOString());
        authLogger.debug('✅ 로그인 데이터 저장 완료');

        // The onAuthStateChanged listener will handle post-authentication
        authLogger.debug('✅ 로그인 성공');
        return userCredential.user;
      } catch (storageError) {
        authLogger.warn('⚠️ 로그인 데이터 저장 실패 (로그인은 성공):', storageError);
        return userCredential.user;
      }
    } catch (error: any) {
  ...
  async signOut(): Promise<void> {
    try {
      authLogger.debug('🔓 로그아웃 시도');

      // Firebase 로그아웃
      await firebaseSignOut(auth);
      authLogger.debug('✅ Firebase 로그아웃 성공');
      ...
      try {
        await firebaseSignOut(auth);
        authLogger.debug('✅ Firebase 강제 로그아웃 성공');
      } catch (firebaseError) {
        authLogger.error('❌ Firebase 강제 로그아웃도 실패:', firebaseError);
      }
    }

    authLogger.debug('✅ 로그아웃 완료');
  }

  // 비밀번호 재설정
  async resetPassword(email: string): Promise<void> {
    try {
      authLogger.debug('🔑 비밀번호 재설정 요청:', email);
      await sendPasswordResetEmail(auth, email);
      authLogger.debug('✅ 비밀번호 재설정 이메일 발송 완료');
    } catch (error: any) {
      authLogger.error('❌ 비밀번호 재설정 실패:', error);
      throw error;
    }
  }
  ...
  async getIdToken(): Promise<string | null> {
    try {
      ...
      try {
        ...
        authLogger.debug('🔄 캐시된 토큰 없음, 새 토큰 요청');
      }

      // 캐시된 토큰이 없거나 실패하면 새 토큰 요청 (true)
      try {
        authLogger.debug('🔄 새 ID 토큰 요청 중...');
        const freshToken = await getIdToken(currentUser, true);
        authLogger.debug('✅ 새 ID 토큰 획득 성공');
        return freshToken;
      } catch (refreshError) {
        authLogger.error('❌ 새 토큰 요청 실패:', refreshError);

        // 로컬 스토리지에서 저장된 사용자 정보를 fallback으로 시도
        try {
          const storedUser = localStorage.getItem('memobee_firebase_user');
          if (storedUser) {
            authLogger.debug('💾 저장된 사용자 정보를 fallback으로 확인');
            // 토큰은 제공할 수 없지만 사용자 정보는 유지
          }
        } catch (storageError) {
          authLogger.error('❌ 저장된 사용자 정보 가져오기도 실패:', storageError);
        }

        throw refreshError;
      }
    } catch (error) {
      authLogger.error('❌ ID 토큰 가져오기 완전 실패:', error);
      return null;
    }
  }
  ...
  private handleAuthError(operation: string, error: AuthError) {
    ...
    switch (error.code) {
      case 'auth/cancelled':
      case 'auth/popup-closed-by-user':
        authLogger.debug(`🚫 사용자가 ${operation}을 취소했습니다.`);
        break;
      case 'auth/popup-blocked':
        authLogger.error('팝업이 차단되었습니다. 브라우저 설정을 확인해주세요.');
        break;
      case 'auth/user-not-found':
        authLogger.error('존재하지 않는 사용자입니다.');
        break;
      case 'auth/wrong-password':
        authLogger.error('잘못된 비밀번호입니다.');
        break;
      case 'auth/email-already-in-use':
        authLogger.error('이미 사용 중인 이메일 주소입니다.');
        break;
      case 'auth/weak-password':
        authLogger.error('비밀번호가 너무 약합니다.');
        break;
      case 'auth/invalid-email':
        authLogger.error('유효하지 않은 이메일 주소입니다.');
        break;
      case 'auth/network-request-failed':
        authLogger.error('네트워크 연결을 확인해주세요.');
        break;
      default:
        authLogger.error(`알 수 없는 ${operation} 오류:`, error.code);
    }
  }
